      ...
      <section className="flex xsm:flex-col sm:flex-row">
        ...
        <div className={`${theme === 'light' ? 'flex w-full flex-col bg-[#e4e6ed]' : 'flex w-full  flex-col bg-main-black'}`}>
          <main className={`flex-1 w-full flex m-auto xl:flex-row xl:max-w-full max-w-[820px] flex-col ${theme === 'light' ? "bg-[#e4e6ed]" : "bg-main-black"}`}>
            <section className="flex-1 xsm:p-1 md:p-4">
...
const SearchInput = ({ query, handleUserSearch }: SearchInputProps) => {
  ...
  return (
    ...
    <>
      <div className={`flex ${show ? 'xsm:flex' : 'xsm:hidden'} sm:flex  rounded-sm shadow-md border-2  ${theme === 'light ' ? 'bg-white border-[#555]' : 'bg-[#333] border-[#555]'} items-center mb-4  overflow-hidden`}>
        <form onSubmit={onSubmit} className={`${theme === 'light' ? 'bg-white hover:bg-[#efefef]' : 'bg-[#222] hover:bg-[#444]'} flex items-center flex-1`}>
